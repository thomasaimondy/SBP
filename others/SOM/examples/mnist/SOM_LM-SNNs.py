...
parser.add_argument("--plot_interval", type=int, default=60)
    ...
    for step, batch in enumerate(dataloader):
        ...
        if step > 0:
            ...
            if step % update_interval == 0:
                ...
                accuracy_all = torch.sum(label_tensor.long() == all_activity_pred).item() / len(label_tensor)
                accuracy_proportion = torch.sum(label_tensor.long() == proportion_pred).item() / len(label_tensor)
                ...
                f = open('logs/SOM_' + str(date) + '_' + 'MNIST_' + str(args.seed) + '.txt', 'a+')
                f.write(str(step) + ' ' + str(accuracy_all) + ' ' + str(accuracy_proportion) + '\n')
                f.close()
