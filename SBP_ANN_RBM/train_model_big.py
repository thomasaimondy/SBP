...
def train_mnist_nonsp(theta_sbp):
    # TRAIN A NETWORK WITH 1 HIDDEN LAYER
    # MNIST
    # for theta in range(21):
    nowtime = time.strftime('-%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
...
def train_nettalk_sp(theta_sbp):
    nowtime = time.strftime('-%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
    # theta_sbp = 0.5
    tasktype='nettalk'
    netT = "net-"+str(theta_sbp)+str(nowtime), {
...
def train_nettalk_nonsp(theta_sbp):
    nowtime = time.strftime('-%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
    # theta_sbp = 0.5
    tasktype='nettalk'
    netT = "net-"+str(theta_sbp)+str(nowtime), {
...
def train_gesture_sp(theta_sbp):
    # TRAIN A NETWORK WITH 1 HIDDEN LAYER
    nowtime = time.strftime('-%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
    # theta_sbp = 0.5
    tasktype = "gesture"
...
def train_gesture_nonsp(theta_sbp):
    # TRAIN A NETWORK WITH 1 HIDDEN LAYER
    nowtime = time.strftime('-%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
    # theta_sbp = 0.5
    tasktype = "gesture"
...
# task 1
theta_sbp = 0.5
train_mnist_sp(theta_sbp)
train_mnist_nonsp(theta_sbp)
train_nettalk_sp(theta_sbp)
train_nettalk_nonsp(theta_sbp)
train_gesture_sp(theta_sbp)
train_gesture_nonsp(theta_sbp)


# task 2
for i in range(10):
    theta_sbp = 0.1 + i*0.01
    train_mnist_sp(theta_sbp)
    train_nettalk_sp(theta_sbp)
    train_gesture_sp(theta_sbp)
