%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% FOR SNN TRAINING WITH SBP, Paralleling {Tasks, layers, learning rates}
...
addpath(genpath('..'));
seed = 3
rand('seed',seed); %100,0,42,512,3,20

%tasks = {{'mnist',[784 10], [0.1,0.05]},{'mnist',[784 500 10],[0.4,0.1,0.01]},{'mnist',[784 500 500 10],[0.4,0.1,0.1,0.01]},{'mnist',[784 500 500 500 10],[0.4,0.1,0.1,0.1,0.01]}, ...
%    {'nettalk',[189 26],[0.1,0.05]},{'nettalk',[189 500 26],[0.4,0.1,0.01]},{'nettalk',[189 500 500 26],[0.4,0.1,0.1,0.01]},{'nettalk',[189 500 500 500 26],[0.4,0.1,0.1,0.1,0.01]}, ...
%    {'guesture',[1024 11],[0.1,0.05]},{'guesture',[1024 500 11],[0.4,0.1,0.01]},{'guesture',[1024 500 500 11],[0.4,0.1,0.1,0.01]},{'guesture',[1024 500 500 500 11],[0.4,0.1,0.1,0.1,0.01]}};

tasks = {{'guesture',[1024 500 11],[0.4,0.1,0.01]}};



parpool('local',length(tasks));

parfor ii = 1:length(tasks)
    % Make log file
    datestr1 = datestr(now, 'yyyymmddTHHMMSS');
    fname_txt =['../logs/log_', datestr1, '_', tasks{ii}{1},'_layers_',num2str(length(tasks{ii}{2})),'_', num2str(seed),'.txt'];
    fname_model = ['../models/snn_' datestr1 '_' tasks{ii}{1} '_layers_' num2str(length(tasks{ii}{2})) '_' num2str(seed) '.mat'];
    fname_figpath = ['../figures/' datestr1  '_' tasks{ii}{1} '_layers_' num2str(length(tasks{ii}{2})) '_' num2str(seed) '/'];
    ...
    switch opts.task
        ...
        case 'nettalk'
            opts.architecture = tasks{ii}{2};
            opts.batchsize=1;
            data = load('../data/nettalk_small.mat');
            ...
            opts.proportion =1;
        case 'guesture'
            opts.architecture = tasks{ii}{2}; %[1024 500 11];
            opts.batchsize=50;
            data1 = load('../data/DVS_gesture_100.mat'); % codes in guesture folder
            train_x = data1.train_x;
            test_x = data1.test_x;
            data2 = load('../data/gesture_label.mat');
    ...
    opts.alpha=tasks{ii}{3}; % learning rates in different layers
